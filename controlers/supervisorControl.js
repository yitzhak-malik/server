...
function supervisorControl() {
   ...
   function createClass(req, res) {
      console.log('hi this creat class');
      if (!req.body.nameClass && !req.body.academic && !req.body.namesInterns[0]) {
         console.log('ksksk');
         return res.status(400).send()
      }
      supervisorSchema.findById(req.user._idS).populate({
         path: 'academics', match: { name: req.body.academic },
         populate: {
            path: 'interns', match: { id: { '$in': req.body.namesInterns } },
            populate: { path: 'typeUser' }
         }
      }).exec(
         (err, supervisor) => {
            if (err) {
